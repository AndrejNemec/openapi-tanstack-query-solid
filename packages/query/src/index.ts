...
const getQueryOptionsDefinition = ({
  ...
  if (type) {
    ...
    return `${partialOptions ? 'Partial<' : ''}${prefix}${pascal(
      ...
      type === QueryType.INFINITE && queryParam && queryParams
  ...
  ${
   ...
   return  ${
     !queryOptionsMutator
       ...
       : 'customOptions'
   } as ${queryOptionFnReturnType} ${'& { initialData?: undefined }'}
}`;

  const operationPrefix = 'use';
...
${doc}export const ${camel(
  ...
  const query = ${camel(`${operationPrefix}-${type}`)}(() => {
    const opts = options?.() || {};
    return ${queryOptionsFnName}(${
    queryProperties && queryProperties.length
      ? queryProperties
          .trim()
          .split(',')
          .map((item) => `opts['${item}']`)
...
${mutationOptionsFn}
    ...
    TContext = unknown>(${mutationProps}) => {
      ...
      return ${operationPrefix}Mutation(() => {
        const opts = ${isRequestOptions ? 'options' : 'mutationOptions'}?.();
...
const dependenciesBuilder: Record<'solid-query', ClientDependenciesBuilder> = {
